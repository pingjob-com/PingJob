  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(csvFilePath)
      ...
      .on('end', async () => {
        ...
        try {
          // Process jobs in batches of 50 for better reliability
          const batchSize = 50;
          let successCount = 0;
          let errorCount = 0;
          ...
          for (let i = 0; i < jobs.length; i += batchSize) {
            ...
            try {
              // Insert batch using parameterized queries for safety
                ...
                try {
                  await pool.query(`
                    ...
                    ON CONFLICT (id) DO UPDATE SET
                      ...
                      updated_at = NOW()
                  `, [
                    job.id, job.company_id, job.title, job.requirements, job.category_id,
                    job.description, job.country, job.state, job.city, job.zip_code,
                    job.status, job.employment_type, job.experience_level, job.salary,
                    job.is_active, 'admin'
                  ]);
                  successCount++;
                } catch (jobError) {
